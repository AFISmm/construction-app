...
export default function VendorsPage() {
  ...
  async function handleDelete(id: number) {
    if (!confirm(t("ven_confirm_del", lang))) return;
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-white">{t("ven_title", lang)}</h1>
          <p className="text-gray-400 text-sm mt-0.5">{vendors.length} {t("ven_count", lang)}</p>
        </div>
      ...
      {showForm && (
        <form onSubmit={handleAdd} className="bg-gray-900 border border-gray-800 rounded-xl p-4 mb-6 grid grid-cols-2 gap-4">
          <div className="col-span-2">
            <label className="text-xs text-gray-400 mb-1 block">{t("ven_form_company", lang)} *</label>
          ...
          <div>
            <label className="text-xs text-gray-400 mb-1 block">{t("ven_form_contact", lang)}</label>
            <input type="text" value={form.contact_name}
              onChange={e => setForm(f => ({ ...f, contact_name: e.target.value }))}
              className="w-full bg-gray-800 border border-gray-700 rounded px-3 py-2 text-white text-sm focus:outline-none focus:border-orange-500" />
          </div>
          <div>
            <label className="text-xs text-gray-400 mb-1 block">{t("ven_form_phone", lang)}</label>
            <input type="text" value={form.phone}
              onChange={e => setForm(f => ({ ...f, phone: e.target.value }))}
              className="w-full bg-gray-800 border border-gray-700 rounded px-3 py-2 text-white text-sm focus:outline-none focus:border-orange-500" />
          </div>
          <div>
            <label className="text-xs text-gray-400 mb-1 block">{t("ven_form_email", lang)}</label>
            <input type="email" value={form.email}
              onChange={e => setForm(f => ({ ...f, email: e.target.value }))}
              className="w-full bg-gray-800 border border-gray-700 rounded px-3 py-2 text-white text-sm focus:outline-none focus:border-orange-500" />
          </div>
          <div>
            <label className="text-xs text-gray-400 mb-1 block">{t("ven_form_trade", lang)}</label>
            <input type="text" value={form.trade}
              onChange={e => setForm(f => ({ ...f, trade: e.target.value }))}
              className="w-full bg-gray-800 border border-gray-700 rounded px-3 py-2 text-white text-sm focus:outline-none focus:border-orange-500" />
          </div>
          <div>
            <label className="text-xs text-gray-400 mb-1 block">{t("ven_form_nit", lang)}</label>
            <input type="text" value={form.nit}
              onChange={e => setForm(f => ({ ...f, nit: e.target.value }))}
              className="w-full bg-gray-800 border border-gray-700 rounded px-3 py-2 text-white text-sm focus:outline-none focus:border-orange-500" />
          </div>
          <div className="col-span-2">
            <label className="text-xs text-gray-400 mb-1 block">{t("ven_form_notes", lang)}</label>
          ...
          <div className="col-span-2 flex gap-3 justify-end">
            ...
            <button type="submit" disabled={saving}
              ...
              {saving ? t("ven_btn_saving", lang) : t("ven_btn_save", lang)}
            </button>
          </div>
        </form>
      )}
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
        <div className="grid grid-cols-[2fr_1.5fr_1fr_1fr_1fr_auto] gap-4 px-4 py-3 border-b border-gray-800 text-xs text-gray-400 uppercase tracking-wider">
          <span>{t("ven_col_company", lang)}</span>
          <span>{t("ven_col_contact", lang)}</span>
          <span>{t("ven_col_trade",   lang)}</span>
          <span>{t("ven_col_phone",   lang)}</span>
          <span>{t("ven_col_status",  lang)}</span>
          <span className="w-8" />
        </div>

        {vendors.length === 0 ? (
          <p className="px-4 py-6 text-gray-500 text-sm">{t("ven_no_data", lang)}</p>
        ) : (
          vendors.map(v => (
            <div key={v.id} className="grid grid-cols-[2fr_1.5fr_1fr_1fr_1fr_auto] gap-4 px-4 py-2.5 border-t border-gray-800/50 text-sm items-center hover:bg-gray-800/20">
              ...
              <select value={v.status} onChange={e => handleStatus(v.id, e.target.value)}
                ...
                <option value="pending">{t("ven_status_pending",  lang)}</option>
                <option value="active">{t("ven_status_active",   lang)}</option>
                <option value="inactive">{t("ven_status_inactive", lang)}</option>
              </select>
