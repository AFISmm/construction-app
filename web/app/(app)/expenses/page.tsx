  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-0.5">
            {expenses.length} payments · Total: <span className="text-white font-semibold">{fmtMoney(grandTotal)}</span>
          </p>
        </div>
        <button
      ...
      {showForm && (
        <form onSubmit={handleAdd} className="bg-gray-900 border border-gray-800 rounded-xl p-4 mb-6 grid grid-cols-2 gap-4">
          <div className="col-span-2">
            <label className="text-xs text-gray-400 mb-1 block">Budget Line *</label>
            <select
              required
          ...
          <div className="col-span-2 flex gap-3 justify-end">
            <button type="button" onClick={() => setShowForm(false)}
              className="text-sm text-gray-400 hover:text-white px-4 py-2 rounded-lg">Cancel</button>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
        ...
        {expenses.length === 0 ? (
          <p className="px-4 py-6 text-gray-500 text-sm">No payments recorded yet.</p>
        ) : (
          expenses.map(exp => (
            <div key={exp.id}
