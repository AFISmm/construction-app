...
class UserPermission(Base):
    ...
    approval_modules: Mapped[Optional[str]] = mapped_column(Text)  # JSON list of modules this user can approve
    created_at: Mapped[datetime] = mapped_column(DateTime, default=func.now(), nullable=False)
    updated_at: Mapped[datetime] = mapped_column(DateTime, default=func.now(), onupdate=func.now(), nullable=False)
...
class Budget(Base):
    ...
    __table_args__ = (
        CheckConstraint("status IN ('draft','review','approved','rejected')", name="chk_budget_status"),
    )
    ...
    versions: Mapped[List["BudgetVersion"]] = relationship(back_populates="budget", cascade="all, delete-orphan")
    audit_log: Mapped[List["BudgetAuditLog"]] = relationship(back_populates="budget")


class BudgetVersion(Base):
    __tablename__ = "budget_versions"
    __table_args__ = (
        CheckConstraint("change_type IN ('minor','major')", name="chk_bv_change_type"),
        CheckConstraint("status IN ('draft','review','approved','rejected')", name="chk_bv_status"),
    )
    ...
    version_label: Mapped[str] = mapped_column(String(20), nullable=False)   # "1.0", "2.3"
    change_type: Mapped[str] = mapped_column(String(10), default="minor", nullable=False)
    change_description: Mapped[Optional[str]] = mapped_column(Text)
    status: Mapped[str] = mapped_column(String(20), default="draft", nullable=False)
    snapshot_json: Mapped[str] = mapped_column(Text, nullable=False)          # full JSON snapshot
    created_at: Mapped[datetime] = mapped_column(DateTime, default=func.now(), nullable=False)
    created_by: Mapped[Optional[int]] = mapped_column(ForeignKey("users.id", ondelete="SET NULL"))

    budget: Mapped["Budget"] = relationship(back_populates="versions")
    audit_entries: Mapped[List["BudgetAuditLog"]] = relationship(back_populates="version")


class BudgetAuditLog(Base):
    """Append-only audit trail — no records are ever deleted."""
    __tablename__ = "budget_audit_log"

    id: Mapped[int] = mapped_column(primary_key=True)
    budget_id: Mapped[int] = mapped_column(ForeignKey("budgets.id", ondelete="CASCADE"), nullable=False)
    version_id: Mapped[Optional[int]] = mapped_column(ForeignKey("budget_versions.id", ondelete="SET NULL"))
    ...
    budget: Mapped["Budget"] = relationship(back_populates="audit_log")
