...
# ── Audit log de versiones de presupuesto ─────────────────────────────────────
from budget_versioning import get_budgets, get_audit_log as _get_audit_log, get_user_email as _get_user_email, STATUS_LABELS_ES as _SL_ES, STATUS_LABELS_EN as _SL_EN
from db import Budget as _Budget
...
with get_session() as _sa:
    _budgets = _sa.query(_Budget).filter_by(project_id=project_id).all()
...
for _b in _budgets:
    for _entry in _get_audit_log(_b.id):
        ...
        _all_audit.append({
            ...
            ("Accion" if _lang == "es" else "Action"):     _ACTION_LABELS.get(_entry.action, _entry.action),
...
if _all_audit:
    _df_audit = _pd.DataFrame(_all_audit)
