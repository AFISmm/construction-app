...
for tab, room in zip(tabs, room_data):
    with tab:
        ...
        c1, c2 = st.columns(2)
        c1.metric(t("room.total_budgeted"), fmt_money(total_bud))
        c2.metric(t("room.total_spent"), fmt_money(total_spent))

        st.subheader(t("room.budget_section"))
        if lines:
            for line in lines:
                spent = get_line_spent(line.id)
                st.write(f"**{line.category_code}** — {line.description or ''} | {fmt_money(float(line.budgeted_amount))} / {fmt_money(spent)}")
        else:
        ...
        if all_expenses:
            for exp in sorted(all_expenses, key=lambda e: e.expense_date, reverse=True):
                st.write(f"{exp.expense_date} | {exp.vendor or '—'} | {fmt_money(exp.amount)}")
        else:
